...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        $sliders = DB::connection('mysql-category')
            ->table('category_slider')
            ->where('c_id', $category->id)
            ->where('status', 1)
            ->get();

        $amazings = DB::connection('mysql-category')
            ->table('category_amazing')
            ->where('c_id', $category->id)
            ->get();

        $banners = DB::connection('mysql-category')
            ->table('category_banner')
            ->where('type', 0)
            ->where('c_id', $category->id)
            ->get();

        $bigbanners = DB::connection('mysql-category')
            ->table('category_banner')
            ->where('type', 1)
            ->where('c_id', $category->id)
            ->take(2)
        ...
        $bigbanners2 = DB::connection('mysql-category')
            ->table('category_banner')
            ->where('type', 1)
            ->skip(2)
            ->where('c_id', $category->id)
        ...
        $title_count = DB::connection('mysql-category')
            ->table('category_title_swiper')
            ->where('c_id', $category->id)
            ->get();

        $products = DB::connection('mysql-category')
            ->table('category_product_swiper')
            ->where('c_id', $category->id)
            ->where('status', 1)
            ->get();

        $brands = DB::connection('mysql-category')
