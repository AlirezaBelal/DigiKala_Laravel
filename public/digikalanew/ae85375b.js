...
var IndexAction = {
                ...
                $.map(response, function (item) {
                    ...
                    if (item.id === 'personal') {
                        try {
                            ...
                            // set category id for SNT
                            removeButton = $('.js-remove-recommendation-swiper[data-id=recommendation-' + item.id + ']');
                            if(removeButton.length > 0 && item.category_id){
                                removeButton.data("category-id", item.category_id);
                            }
                        ...
                        if (dom.length > 0) {
                            dom.find('.o-headline').find('.js-carousel-title').html(item.title);
                            if(item.description){
                                var carouselDescription = dom.find('.o-headline').find('.js-carousel-description');
                                carouselDescription.html(item.description);
                                carouselDescription.removeClass('u-hidden');
                            }
                            $.map(item.products, function (product) {
                                if (product.image) {
                                    products += thiz.createTemplate(template, {
            ...
            swipers[i] = new Swiper($promoBox.get(0), {
                ...
                on: {
                    ...
                    slideChange: function () {
                        ...
                        if(isModuleActive('new_data_layer') && window.DataLayerConfig){
                            window.DataLayerConfig.checkProducts();
                        }
                    },
                    resize: function () {
    ...
    setGACarouselsImpressionEvent: function () {
        $(document).on('click', '.js-carousel-ga-product-box', function () {
    ...
    setBannerViewImpressionEvent: function () {
        ...
        $(document).ready(function() { // waiting for back-end data
            // top left banners
            var topLeftBanners = $('.js-ad-placement-column-item');
            if(topLeftBanners.length > 0){
                topLeftBanners.each(function () {
                    try {
                        ...
                        console.log('GA Error: Home Page Top Left Banners View Impression')
                    }
                })
            }
        });
