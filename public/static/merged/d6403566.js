...
var ProductsIndexAction = {
    ...
    init: function () {
        ...
        if(isModuleActive('dk_products_view_impression_event')) {
            this.setProductsViewImpressionEvent();
        }
    },

    AB_initCloseDidYouMean: function(){
        $('.js-close-did-you-mean-box').click(function () {
            $(this).closest('.js-ab-did-you-mean-box').hide();
        })
    },
    ...
    initABAddToWishList: function () {
        $('.js-ab-add-to-wish-list').on('click' , function (event) {
            ...
            var csrfToken = thiz.data('csrf-token');
            ...
            Services.ajaxPOSTRequestJSON(
              '/ajax/favorites/product/add/' + productId + '/',
              {token: csrfToken},
              function () {
    ...
    setProductsViewImpressionEvent: function () {
        $(window).on('scroll', Services.throttle(function() {
            var productsBox = $('.js-product-box[data-observed="0"]');
            productsBox.each(function () {
                if($(this).data('observed') === 1) return;
                try {
                    var productId = $(this).data('id');
                    if(!productId || !Services.isScrolledIntoView(this)) return;
                    var productData = Services.retrieveProductFromDataLayer({
                        productId: productId,
                        eventName: "eec.productImpression",
                        pathArray: ['ecommerce','impressions']
                    ...
                    if (productData){
                        ...
                        this.setAttribute('data-observed' , 1); // require for .js-product-box[data-observed="0"]
                        ga("eec.productImpression", productData);
                    }
                } catch (e) {
                    console.log('GA Error: PLP - Product View Impression')
                }
            })
        } , 500));
    }
