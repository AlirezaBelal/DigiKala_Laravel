<section class="c-swiper c-swiper--products has-placeholder recommendation-swiper js-sntracker-carousel"
    ...
    @php
        $title_4 = \App\Models\TitleCategoryIndex::get()[4];
    @endphp
    <div class="o-headline"><span>{{$title_4->title}}</span></div>
    <div class="c-box">
        <div class="swiper-container swiper-container-horizontal swiper-container-rtl">
            <div class="swiper-wrapper js-products-container" id="js-products-container5" style="transform: translate3d(0px, 0px, 0px);">
                @foreach(\App\Models\CategoryIndex::where('title_id',$title_4->id)->where('status',1)->get() as $category)
...
$per = ($difPrice * 100) / $category->product->price;
            ...
            <div class="swiper-button-prev js-swiper-button-prev swiper-button-disabled5" id="swiper_button_disabled5"
                 onclick="js_swiper_button_prev5()"></div>
            <div class="swiper-button-next  js-swiper-button-next"  id="swiper_button_add5"
                 onclick="js_swiper_button_next5()"></div>
        </div>
    </div>
</section>
<script>
    function js_swiper_button_next5() {
        document.getElementById("js-products-container5").style ="transform: translate3d(530px, 0px, 0px);transition-duration: 0ms";
        document.getElementById("swiper_button_disabled5").classList.remove("swiper-button-disabled");
        document.getElementById("swiper_button_add5").style ="opacity: .15; cursor: auto;";

    }
    function js_swiper_button_prev5() {
        document.getElementById("js-products-container5").style ="transform: translate3d(0px, 0px, 0px)";
        document.getElementById("swiper_button_disabled5").classList.remove("swiper-button-disabled");
        document.getElementById("swiper_button_disabled5").style ="opacity: .15; cursor: auto;";
        document.getElementById("swiper_button_add5").style ="opacity: 1; cursor: poiner;";
    }
</script>
